...
namespace sharpPDF
{
	...
	public class pdfDocument
	{
        ...
        /// <summary>
		/// Method that writes the PDF document on the string variable
		/// </summary>
		public string createPDFToString()
        {
            string _myString = "";
            long _bufferLength = 0;
            ...
            {
                //PDF's definition
                _bufferLength += writeToBuffer(ref _myString, @"%PDF-1.4" + Convert.ToChar(13) + Convert.ToChar(10));

                //PDF's header object
                _trailer.addObject(_bufferLength.ToString());
                _bufferLength += writeToBuffer(ref _myString, _header.getText());

                //PDF's info object
                _trailer.addObject(_bufferLength.ToString());
                _bufferLength += writeToBuffer(ref _myString, _info.getText());

                //PDF's outlines object
                _trailer.addObject(_bufferLength.ToString());
                _bufferLength += writeToBuffer(ref _myString, _outlines.getText());

                //PDF's bookmarks
                foreach (pdfBookmarkNode Node in _outlines.getBookmarks())
                {
                    _trailer.addObject(_bufferLength.ToString());
                    _bufferLength += writeToBuffer(ref _myString, Node.getText());
                }
                ...
                {
                    _trailer.addObject(_bufferLength.ToString());
                    _bufferLength += writeToBuffer(ref _myString, font.getText());
                }
                //PDF's pagetree object
                _trailer.addObject(_bufferLength.ToString());
                _bufferLength += writeToBuffer(ref _myString, _pageTree.getText());

                //Generation of PDF's pages
                foreach (pdfPage page in _pages)
                {
                    _trailer.addObject(_bufferLength.ToString());
                    _bufferLength += writeToBuffer(ref _myString, page.getText());
                    ...
                    foreach (pdfElement element in page.elements)
                    {
                        ...
                        {
                            _trailer.addObject(_bufferLength.ToString());
                            _bufferLength += writeToBuffer(ref _myString, element.getText());
                            _trailer.addObject(_bufferLength.ToString());
                            _bufferLength += writeToBuffer(ref _myString, ((imageElement)element).getXObjectText());
                            _bufferLength += writeToBuffer(ref _myString, "stream" + Convert.ToChar(13) + Convert.ToChar(10));
                            _bufferLength += writeToBuffer(ref _myString, ((imageElement)element).content);
                            _bufferLength += writeToBuffer(ref _myString, Convert.ToChar(13).ToString());
                            _bufferLength += writeToBuffer(ref _myString, Convert.ToChar(10).ToString());
                            _bufferLength += writeToBuffer(ref _myString, "endstream" + Convert.ToChar(13) + Convert.ToChar(10));
                            _bufferLength += writeToBuffer(ref _myString, "endobj" + Convert.ToChar(13) + Convert.ToChar(10));
                        }
                        else {
                            _trailer.addObject(_bufferLength.ToString());
                            _bufferLength += writeToBuffer(ref _myString, element.getText());
                ...
                //PDF's trailer object
                _trailer.xrefOffset = _bufferLength;
                _bufferLength += writeToBuffer(ref _myString, _trailer.getText());
		...
		/// Method that writes into the buffer a string
		/// </summary>
		/// <param name="myString">Output String</param>
		/// <param name="stringContent">String that contains the informations</param>
		/// <returns>The number of the bytes written in the Buffer</returns>
		private long writeToBuffer(ref string myString, string stringContent)
        {
            //ASCIIEncoding myEncoder = new ASCIIEncoding();
            byte[] arrTemp;
            try
            {
                //arrTemp = myEncoder.GetBytes(stringContent);
                arrTemp = Encoding.Default.GetBytes(stringContent);
                string base64 = Convert.ToBase64String(arrTemp);
                byte[] base64B = Convert.FromBase64String(base64);
                //myString = myString + stringContent;
                myString = myString + base64;
                //return arrTemp.Length;
                return base64B.Length;
            }
            catch (IOException ex)
            {
                throw new pdfBufferErrorException("Errore nella scrittura del Buffer", ex);
        ...
        /// <param name="byteContent">A Byte array that contains the informations</param>
        /// <returns>The number of the bytes written in the Buffer</returns>
        private long writeToBuffer(ref string myString, byte[] byteContent)
        {
            try
            {
                myString = myString + Convert.ToBase64String(byteContent);
                return byteContent.Length;
            }
            catch (IOException ex)
            {
